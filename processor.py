"""
This script pulls the final dataframe and compresses the data asyncronously into figures
"""
...
async def distributions():
    ...
    with option_context(
        ...
        try:
            data = final[
                ["Size_Bytes", "Category"]
            ].to_pandas()  # .sort_values(by='Size_Bytes')
        except:
            print("Size is null")
        distFig1 = px.box(
            data,
            ...
            y="Size_Bytes",
            color="Category",
            title="Distribution by Category - Double Click on Legend to Isolate Trace",
        )
...
async def firsts():
    ...
    with option_context(
        ...
        try:
            data = (
                final.groupby(["Category", "App_Name"])["Released"]
                .min()
        ...
        firstsFig1 = px.line(
            ...
            y="Count",
            color="Category",
            title="First Releases by App - Time Series",
        )
...
async def releases():
    ...
    with option_context(
        ...
        try:
            data = (
                final.groupby(["Released", "Category"])
                .size()
                .to_frame("New_Releases")
                .reset_index()
        ...
        releasedFig1 = px.line(
            ...
            y="New_Releases",
            color="Category",
            title="New Releases - Time Series",
        )
...
async def ratings():
    ...
    with option_context(
        ...
        score = (
            score.groupby(["Category"])["Weighted_Rating"]
            .sum()
            .to_frame("Weighted_Rating")
            .reset_index()
            .to_pandas()
        )
...
async def biggests():
    ...
    with option_context(
        ...
        try:
            ...
            bigs["Rank"] = bigs.groupby(["Released_Year", "Category"])[
                "Size_Bytes"
        ...
        biggestsFig1 = go.Figure(
            data=[
                go.Table(
                    header=dict(values=list(bigs.columns)),
                    cells=dict(
                        values=[
                            bigs.App_Name,
                            bigs.Category,
...
loop.run_until_complete(
    asyncio.gather(distributions(), releases(), ratings(), biggests(), firsts())
)
